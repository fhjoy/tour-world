...
const multerFilter = (req, file, cb) => {
  if (file.mimetype.startsWith('image')) {                  // mimetype = "iamge/jpeg" for images
    cb(null, true);
  } else {
...
exports.resizeUserPhoto = catchAsync(async (req, res, next) => {  // middleware for resize photo after uploading the photo
...
const filterObj = (obj, ...allowedFields) => {                    // ...allowedFields getting string and mkning it an array.
  const newObj = {};
...
exports.updateMe = catchAsync(async (req, res, next) => {    // console.log (req.file) console.log (req.body). from where we will get file, and mimetype. // updateMe is for currenly authenticated user.
...
exports.deleteMe = catchAsync(async (req, res, next) => {        // for deleting the account. but actually its not deleted from the databse we just deactivate the account.
